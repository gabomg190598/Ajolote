		...
		Try {
				...
				if ($ShowProgress) {
					...
					if (Test-Path $LockScreen) {
						$supPath=Join-Path $Env:TEMP (New-GUID).guid
						if (!(Test-Path -Path $supPath -PathType Container)) {
							New-Item -Path $supPath -ItemType Directory -Force | Out-Null
						}
						$pgstatus = "$($supPath)\status.ini"
						$HPUI="$($supPath)\HPFullLockScreen.exe"
						Copy-Item -Path $LockScreen -Destination $HPUI -Force
						"$($strMode)$($strFile)"|Out-File $pgstatus -Encoding default -Force
						"[progress] 0%" |Out-File $pgstatus -Encoding default -Append -force
						while (Get-Process | Where-Object {$_.Name -eq "HPFullLockScreen"}) {
							Get-Process | Where-Object {$_.Name -eq "HPFullLockScreen"} | Stop-Process -Force -ErrorAction SilentlyContinue
							Start-Sleep -Seconds 5
							WriteLog -Message "Closing HPFullLockScreen app..." -Component $MyInvocation.MyCommand.Name
						}
						if (Test-Path -Path $HPUI -PathType Leaf) {
							$runexe = Start-Process $HPUI -PassThru -WorkingDirectory $supPath -ErrorAction SilentlyContinue
							WriteLog -Message "`tCall Look screen, path: [$($supPath)] Id:[$($runexe.Id)]" -Component $MyInvocation.MyCommand.Name
						} else {
							WriteLog -Message "`tMissing Look screen, path: [$($supPath)]" -Messagetype Error -Component $MyInvocation.MyCommand.Name
						}
					} else {
						$gethta = BuildHTA
						...
						if ($Event.MessageData.showp) {
							try {
								...
								if ($logline.Contains("%")){  #used for application or where % value exists
									...
									Write-host "Waiting Dism response..."
									#Write-host "No % found"
								}
			...
			Catch 
			{
				...
				WriteLog -Message "========================================================================================" -Component $MyInvocation.MyCommand.Name
				WriteLog -Message "Exception: $($Error[0].Exception)" -MessageType Error -Component $MyInvocation.MyCommand.Name
				WriteLog -Message "Position: $($Error[0].InvocationInfo.PositionMessage)" -MessageType Error -Component $MyInvocation.MyCommand.Name
				$Error | Out-File -FilePath "$($OutFile).err" -Encoding ascii -Force
				WriteLog -Message "========================================================================================" -Component $MyInvocation.MyCommand.Name
				if (Test-Path $pgf) {Remove-Item $pgf -Force}
				if (Test-Path $pgmessage) {Remove-Item $pgmessage -Force}
				#if (Test-Path $pgstatus) {Remove-Item $pgstatus -Force}
