...
if ($null -ne $json.JOBREQUEST.Java) { 
    if ($null -ne $json.JOBREQUEST.Java.status) {
        ...
        if ($json.JOBREQUEST.Java.status.ToLower().Trim() -eq "new") {
            ...
            if ((Test-Path -Path $strSW_Path -PathType Container) -AND ($null -ne (Get-ChildItem -Path $strSW_Path -Filter "*.exe"))) {
                if ($null -ne (Get-ChildItem -Path $strSW_Path -Filter "*.exe" -File)) {
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.Java "verify" "Waiting for validation"
                }
                ...
                if ($null -eq $DriveComponents) {
                    ...
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.Java "fail" $global:MessageResult
        ...
        } elseif ($json.JOBREQUEST.Java.status.ToLower().Trim() -eq "verify") {
