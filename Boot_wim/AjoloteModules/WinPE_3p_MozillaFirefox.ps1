...
if ($null -ne $json.JOBREQUEST.FireFox) { 
    if ($null -ne $json.JOBREQUEST.FireFox.status) {
        ...
        if ($json.JOBREQUEST.FireFox.status.ToLower().Trim() -eq "new") {
            ...
            if ((Test-Path -Path $strSW_Path -PathType Container) -AND ($null -ne (Get-ChildItem -Path $strSW_Path -Filter "*.exe"))) {
                if ($null -ne (Get-ChildItem -Path $strSW_Path -Filter "*.exe" -File)) {
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.FireFox "verify" "Waiting for validation"
                }
                ...
                if ($null -eq $DriveComponents) {
                    ...
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.FireFox "fail" $global:MessageResult
        ...
        } elseif ($json.JOBREQUEST.FireFox.status.ToLower().Trim() -eq "verify") {
