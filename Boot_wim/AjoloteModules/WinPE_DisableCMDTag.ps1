   ...
   if (($null -ne $json.JOBREQUEST.DisableCMDTag.status) -AND ($json.JOBREQUEST.DisableCMDTag.status.ToLower()) -eq "new") {
    ...
    ##Validate image that is ready to be saved and perform the file creation
        ...
        if ($json.JOBREQUEST.Job.status -eq "save") {
            ...
            {
                WriteLog -Message "Path does not exist... Creating Path"
                New-Item -Path "$($OsDrive)\Windows\Setup\Scripts" -Name "DisableCMDRequest.tag" -ItemType File -Value "Disable CMD"
                WriteLog -Message "==============DisableCMDRequest.tag created SUCCESSFULLY=============="
            }
             ## PASS RESULT
             if (Test-Path -Path "$($OsDrive)\Windows\Setup\Scripts\DisableCMDRequest.tag") {
                ...
                WriteLog -Message "Path exists... Creating DisableCMDRequest.Tag"
                New-Item -ItemType Directory -Force -Path "$($OsDrive)\Windows\Setup\Scripts"
                WriteLog -Message "Path: $($OsDrive)\Windows\Setup\Scripts created Successfully"
                if(!(Test-Path -Path "$($OsDrive)\Windows\Setup\Scripts\DisableCMDRequest.tag")){
            ...
            Update-JobStatus $jobfile $json $json.DisableCMDTag "pass" "Tag created succesfully"
