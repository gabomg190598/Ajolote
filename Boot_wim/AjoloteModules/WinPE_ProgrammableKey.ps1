...
Version 1.0.1
    Date: 3/21/2024
    Root node: $json.JOBREQUEST.HPProgrammableKey
    value: status
        "new", "fail", "pass"
    ...
    Value: reboot
        flag to mark as already installed
    Value: error
        Out message
#>

$SW_Name="HP Programmable Key"
$LogFile=$SW_Name.Replace(" ","").Replace("(","").Replace(")","")+".log"

if ($null -ne $json.JOBREQUEST.HPProgrammableKey) {
    WriteLog -Message "This module is required, checking conditions..." -Verbose
    if ((($null -ne $json.JOBREQUEST.Control) -AND ($json.JOBREQUEST.Control.status -eq "save")) -OR (($null -ne $json.JOBREQUEST.Job) -AND ($json.JOBREQUEST.Job.status -eq "save"))) {
        WriteLog -Message "Image is ready to save, checking status" -Verbose
        if ($null -ne $json.JOBREQUEST.HPProgrammableKey.status) {
            if ($json.JOBREQUEST.HPProgrammableKey.status.ToLower() -eq "new") {                
                ...
                if ($null -eq $json.JOBREQUEST.Drivers.sysid) { 
                    ...
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.HPProgrammableKey "fail" $global:MessageResult
                ...
                if ($arrFilterCVAs.Count -gt 0 ) {
                    ...
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.HPProgrammableKey "fail" $global:MessageResults
                    Out-WinPE -Backuplogs -RemoveJob 
                }
                ...
                if (($null -ne $json.JOBREQUEST.HPProgrammableKey.reboot) -AND ($json.JOBREQUEST.HPProgrammableKey.reboot)) {                        
                    WriteLog -Message "For uknown reason this process is runnign again, $($GetCVA.Title), abort process to review logs" -Verbose
                    $global:MessageResults = "For uknown reason this process is runnign again, $($GetCVA.Title), abort process to review logs"
                ...
                #it is mandatory for now, that PPSolution been added, confirm it
                if (($null -eq $json.JOBREQUEST.AddPPSolution) -OR (-Not($json.JOBREQUEST.AddPPSolution))) {
                   ...
                   #Confirm that HPComplete.exe is present on C:\Windows\Setup\Scripts\
                    if (-Not(Test-Path (Join-Path $OSDrive "\Windows\Setup\Scripts\HPComplete.exe") -PathType Leaf)) {
                        WriteLog -Message "Copying HPComplete to OS drive" -Verbose
                ...
                Update-JobStatus $jobfile $json $json.JOBREQUEST.HPProgrammableKey "pass" "$($SW_Name) has been prepared for post processing"


            } elseif ($json.JOBREQUEST.HPProgrammableKey.status.ToLower() -eq "pass") { 
                WriteLog -Message "$($SW_Name) already installed as successfully, status [$($json.JOBREQUEST.HPProgrammableKey.status)]" -Verbose
            } elseif ($json.JOBREQUEST.HPProgrammableKey.status.ToLower() -eq "fail") {
                WriteLog -Message "$($SW_Name) detected status [$($json.JOBREQUEST.HPProgrammableKey.status)], abort process" -Message Error -Verbose
                $global:MessageResults = "$($SW_Name) detected status [$($json.JOBREQUEST.HPProgrammableKey.status)], abort process"
                $global:CodeResults = 6
                Out-WinPE -Backuplogs -RemoveJob
            } else {
                WriteLog -Message "It is not expected to detect status [$($json.JOBREQUEST.HPProgrammableKey.status)], abort process" -Message Error -Verbose
                $global:MessageResults = "It is not expected to detect status [$($json.JOBREQUEST.HPProgrammableKey.status)], abort process"
                $global:CodeResults = 5
                Update-JobStatus $jobfile $json $json.JOBREQUEST.HPProgrammableKey "fail" $global:MessageResult
                Out-WinPE -Backuplogs -RemoveJob
            }
        } else {
            WriteLog -Message "$($SW_Name) module is required in job, however status value is missing" -MessageType Error -Verbose
            Update-JobStatus $jobfile $json $json.JOBREQUEST.HPProgrammableKey "fail" "$($SW_Name) module is required in job, however status value is missing"
            $global:MessageResults = "$($SW_Name) module is required in job, however status value is missing"
