...
$SW_Title = ".Net Framework 3.5"
$SW_Folder = "WinFeature"
if (($null -ne $json.JOBREQUEST.EnableNetFX35) -AND ($json.JOBREQUEST.EnableNetFX35)) { 
    ...
    $RequireEnable=$false
    if (!(Test-Path 'HKLM:\HPImg\Microsoft\NET Framework Setup\NDP\v3.5\')) {
        $RequireEnable=$true
    }
    ...
    if ($RequireEnable) {
        ...
        if ($null -eq $DriveComponents) {
            ...
            $EnableNetFX = Invoke-RunPower -File "Dism.exe " -Params "/Image:$($OSDrive)\ /ScratchDir:$($OSDrive)\ /enable-feature /featurename:NetFX3 /All /Source:""$((Join-Path (Join-Path $DriveComponents $SW_Folder) $sourcefolder))\sources\sxs"" /LimitAccess" -WorkDir $PSScriptRoot -OutFile "$($Logs)\EnableNetFX35.log" -Verbose
            if ($EnableNetFX -ne 0) { 
                ...
                $global:CodeResults=$EnableNetFX
                Out-WinPE -Backuplogs -RemoveJob
            }
