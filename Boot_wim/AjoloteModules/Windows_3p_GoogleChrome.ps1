...
$GoogleChrome_SilentParameters="/silent /install"
$strSW_Path=(Join-Path $Env:SystemDrive "\SWSETUP\Google_Chrome")
$SW_Title="Google Chrome"
$strLogfile="Setup_GoogleChrome.log"
...
if ($null -ne $json.JOBREQUEST.GoogleChrome) { 
    if ($null -ne $json.JOBREQUEST.GoogleChrome.status) {
        ...
        if ($json.JOBREQUEST.GoogleChrome.status.ToLower().Trim() -eq "verify") {
            ...
            if (Test-Path -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Google Chrome") {
                WriteLog -Message "Detected $($SW_Title) installed on this system, checking details" -Verbose
                $GetApp=@{}
                    $GetApp.Name=(Get-ItemProperty -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Google Chrome" -Name "DisplayName")."DisplayName"
                    $GetApp.Version=(Get-ItemProperty -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Google Chrome" -Name "DisplayVersion")."DisplayVersion"
            }   
            if ($null -eq $GetApp) {
                ...
                if (-Not(Test-Path -Path $strSW_Path -PathType Container)) {
                    ...
                    Update-JobStatus $jobfile $json $json.JOBREQUEST.GoogleChrome "fail" $global:MessageResults
                    Out-Windows 
                }
                ...
                $RunSetup = Invoke-RunPower -file "cmd.exe" -Params "/c ""$($Files[0].FullName)"" $($GoogleChrome_SilentParameters)" -WorkDir $strSW_Path -OutFile (Join-Path $logs $strLogfile) -Verbose
                if ($RunSetup -ne 0) {
                ...
                Update-JobStatus $jobfile $json $json.JOBREQUEST.GoogleChrome "pass" "It is detected $($GetApp.Name) with version $($GetApp.Version), on this system"
            } else {                
                WriteLog -Message "It is detected $($GetApp.Name) with version $($GetApp.Version), on this system" -Verbose
                if (Test-Path -Path $strSW_Path -PathType Container) { Remove-Item -Path $strSW_Path -Recurse -Force -ErrorAction SilentlyContinue}
                Update-JobStatus $jobfile $json $json.JOBREQUEST.GoogleChrome "pass" "It is detected $($GetApp.Name) with version $($GetApp.Version), on this system"
        ...
        } elseif ($json.JOBREQUEST.GoogleChrome.status.ToLower().Trim() -eq "new") {
